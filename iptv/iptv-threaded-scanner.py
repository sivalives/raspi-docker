...
def verify_playlist_link(url, timeout, indent=1, check_first_N_only=5):
    ...
    for segment in m3u8_obj.data['segments']:
        if segment.get('uri'):
            ...
            nice_print('URI key not found', indent=indent, debug=True)
            return False
...
if __name__ == '__main__':
    ...
    try:
        playlist_items = filter_streams(args.input_file, args.timeout, args.blacklist_file)
        with concurrent.futures.ThreadPoolExecutor() as executor:
            results= {executor.submit(verify_playlist_item,item,1): item for item in playlist_items} 
            for f in concurrent.futures.as_completed(results):
              item = results[f]
              #print(f'Item = {item}')
              #print(f.result())
              if f.result():
                  filtered_playlist_items.append(item)
